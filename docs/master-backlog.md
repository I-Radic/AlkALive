...
### Status after the implementation waves (2026-08-27)

| Item | Wave-0 status | Resolution (commit) |
|------|---------------|---------------------|
| REQ-008 `reevaluate` | [PARTIAL] | **EXISTS** — composed-cycle API wired into the frame loop (5924fb4, wave-2) |
| REQ-010 TD1 persistent atlas | [PARTIAL] | **EXISTS** — persistent atlas in both backends w/ overflow reset (b3764f8, wave-2) |
| REQ-011 DependencyGraph serialization | [MISSING] | **EXISTS** — `to_json()` + CLI `dep_graph` emission (2f1cbac wave-1 + wave-3) |
| REQ-012 path-halving | [PARTIAL] | **EXISTS** — `find_mut` switched to path halving (1895b2a, wave-2) |
| REQ-013 RewriteRule trait | [PARTIAL] | **EXISTS** — trait + RULES registry driving the fixpoint (1895b2a, wave-2) |
| REQ-018 lint op-scan | [PARTIAL] | **EXISTS** — full illegal-op scan, all 9 ops (ac73bd4, wave-2) |
| ENTITY-002 ThreadAffinity | [PARTIAL] | **EXISTS** — enum + RenderPass.affinity + JSON (5708dc9, wave-1) |
| ENTITY-004 ComputationId | [PARTIAL] | **EXISTS** — documented role alias (3285a5c, wave-1) |
| ENTITY-006 RewriteRule | [PARTIAL] | **EXISTS** — same fix as REQ-013 |
| DOC-001…DOC-006 | drift | **RESOLVED** — spec amended to as-built (ebcb8e2 wave-3; DOC-002/004 carry as-built notes) |
| DOC-007 | drift | **RESOLVED** (d867f1b, wave-5) |
| DOC-008 | naming drift | **NOTED** — kept as-is (consistent with ADR-026/028/deny.toml usage) |

**Post-implementation tally: 93/93 backlog items [EXISTS]; 7/8 DOC items resolved, 1 noted.**
Zero placeholders repo-wide; suite 1,151 passed / 0 failed at wave-5 HEAD.

### Wave-0 snapshot (at gap-analysis time)

| Status | Count | Items |
|--------|-------|-------|
| [MISSING] | **1** | REQ-011 |
| [PARTIAL] | **6** | REQ-008, REQ-010, REQ-012, REQ-013, REQ-018 (+ ENTITY-002/004/006 same fixes) |
| [EXISTS] | 86 | all remaining |
| DOC drift | 8 | DOC-001…DOC-008 |

**Quantified at Wave 0:** 7 of 93 code-level items (7.5%) were missing or partial;
86 of 93 (92.5%) existed with file:line evidence; 8 documentation-drift items
required spec/doc reconciliation. No unimplementable or contradictory
requirement was found; no spec section was skipped.
...
## 9. REQ → Test Mapping (Wave 6 verification)

Every functional requirement carries at least one passing test. Key mappings
(test names searchable in `crates/**`):

| REQ | Covering tests (selection) |
|-----|------------------------------|
| REQ-001/002/014 | pipeline.rs (15 integration), `scheduled_scene_to_json_shape`, `compile_file_*` CLI tests |
| REQ-003/004/005 | schedule.rs 16 tests (`full_hello_world_scene_produces_five_passes`), `hello_alk_backward_compat_*` |
| REQ-006 | backend lib `render_frame_internal` pass-order tests (lib.rs:1856–1867) + render graph.rs pass tests |
| REQ-007 | incremental.rs `incremental_analysis_*` (20 tests) |
| REQ-008 | signal_store.rs `reevaluate_*` (5 tests + doc test) |
| REQ-009 | `empty_scene_is_small_scene`, SMALL_SCENE_THRESHOLD assertions |
| REQ-010 | tessellate.rs `persistent_atlas_*` (3), text `harfrust_glyph_atlas_reset_*` (3) |
| REQ-011 | incremental.rs `dependency_graph_to_json_*` (3), main.rs `scheduled_scene_to_json_shape` |
| REQ-012 | egraph.rs `find_mut_applies_path_halving`, `find_mut_path_halving_shortens_long_chains` |
| REQ-013 | egraph.rs `rules_registry_lists_exactly_the_three_fixpoint_rules`, `rewrite_rule_trait_delegates_to_free_functions`, `fixpoint_via_registry_matches_direct_application` |
| REQ-015/016 | `egraph_optimization_terminates_on_cyclic_input`, `all_rules_report_noop_on_hello_world_graph` |
| REQ-017/018 | lints/monotonicity.rs 19 tests incl. `op_scan_covers_every_documented_operation` |
| REQ-019/020/021 | lint_tests.rs (18 integration), `run_compile_lint_flag_parses_without_error` |
| REQ-022 | typechecker.rs lattice/covariance/method-op tests (70 across three suites) |
| REQ-023/024/025 | lexer/parser/ast P2 tests + phase2_tests.rs (52) |
| REQ-026/027 | `compile_typecheck_*`, seminative.rs 9 tests, runtime `build_scene_from_algorithm` hooks |
| REQ-028 | ADR doc presence (verified by audits) |
| REQ-029 | proofs.rs/z3_backend.rs absence (verified by audits) |
| REQ-030/031 | signal_store + runtime boot tests; `probe_outcome_distinguishes_all_three_outcomes`, `webgpu_probe_timeout_is_ten_seconds` |
| REQ-032 | TD3 fix verified by `reevaluate_*` cycle tests + frame-loop review |
| REQ-033/034/035 | input-forwarding behavior asserted in e2e suites (chromium/firefox); `clamp_dimensions_*` (2) |
| REQ-036/037 | backend lib 21 tests, offscreen_wgpu.rs, frame_plan 9, wgsl_shaders 3 |
| REQ-038/039 | text lib 40 tests (SEC-03/SEC-04, tofu, atlas cache/reset) |
| REQ-040/041 | render lib tests + graph.rs validate/build tests |
| REQ-042 | hello.alk compat tests ×3 |
| REQ-043 | workspace structure asserted by cargo check --workspace + CI |
| REQ-044 | build-deploy pipeline gates in CI (deploy-pipeline job; 50.3% shrink, SHA match) |
| RULE-011 | codegen.rs `lower_text_node_infinite_*` (3), `codegen_error_carries_source_position` |
| RULE-014/015/016 | typechecker + lint deny-upgrade tests |
| RULE-019 | repo-wide placeholder scan (audit 4-a: zero) + panic audit (zero unprotected after wave-4 fix) |
| RULE-020 | workspace default-features mechanism + cargo tree verification (prior wave-2 fix) |

*End of master backlog. Generated by the Implementation Orchestrator, Wave 0; statuses and mapping updated at Waves 5–6.*
